...
class HBTB09Hists{
 ...
 public:
  ...
  void setup(DQMStore* &m_dbe, const std::string& fName, const std::string& fTitle, const std::string& fUnits, 
             int nBins, double xMin, double xMax, int fDepths)
    {
      ...
      for (int i=1; i<=fDepths; i++) {
      
          sprintf(charname,"d%02d_%s",i,fName.c_str());
          sprintf(chartitle,"%s -- Depth %d;%s",fTitle.c_str(),i,fUnits.c_str());
          name = charname;
          title = chartitle;
                
          // Push back depth plots
          depth.push_back(m_dbe->book1D(name,title,nBins,xMin,xMax));
      }
    } // void setup(...)
...
class HBTB09EtaPhiHists{
  // Make a set of eta-phi histograms (one for each depth)
 public:
  HBTB09EtaPhiHists(){};
  ~HBTB09EtaPhiHists(){};

  void setup(DQMStore* &m_dbe, const std::string& fName, const std::string& fTitle, const std::string& fUnits, int fDepths)
    {
      ...
      for (int i=1; i<=fDepths; i++) {
          ...
          sprintf(charname,"h%02d_%s",i,fName.c_str());
          sprintf(chartitle,"%s -- Depth %d;%s",fTitle.c_str(),i,fUnits.c_str());
          name = charname;
          title = chartitle;
                
          // Push back depth plots
          depth.push_back(m_dbe->book2D(name,title,4,5.5,9.5,4,1.5,5.5));
      }
    } // void setup(...)

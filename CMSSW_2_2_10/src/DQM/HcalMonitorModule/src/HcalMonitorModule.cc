...
HcalMonitorModule::HcalMonitorModule(const edm::ParameterSet& ps){
  ...
  zdcMon_ = 0;
  h2blMon_ = 0;    // Testbeam
  h2pedMon_ = 0;   // Testbeam
  h2rhMon_ = 0;    // Testbeam
  h2trigMon_ = 0;  // Testbeam
  ...
  if (ps.getUntrackedParameter<bool>("H2BeamLineMonitor",false)){ // Testbeam
    if(debug_>0) std::cout << "HcalMonitorModule: H2BeamLine monitor flag is on...."<<endl;
    h2blMon_ = new H2BeamLineMonitor();
    h2blMon_->setup(ps, dbe_);
  }
  
  if (ps.getUntrackedParameter<bool>("H2PedestalMonitor",false)){ // Testbeam
    if(debug_>0) std::cout << "HcalMonitorModule: H2Pedestal monitor flag is on...."<<endl;
    h2pedMon_ = new H2PedestalMonitor();
    h2pedMon_->setup(ps, dbe_);
  }
  
  if (ps.getUntrackedParameter<bool>("H2RecHitMonitor",false)){ // Testbeam
    if(debug_>0) std::cout << "HcalMonitorModule: H2RecHit monitor flag is on...."<<endl;
    h2rhMon_ = new H2RecHitMonitor();
    h2rhMon_->setup(ps, dbe_);
  }
  
  if (ps.getUntrackedParameter<bool>("H2TriggerMonitor",false)){ // Testbeam
    if(debug_>0) std::cout << "HcalMonitorModule: H2Trigger monitor flag is on...."<<endl;
    h2trigMon_ = new H2TriggerMonitor();
    h2trigMon_->setup(ps, dbe_);
  }
...
HcalMonitorModule::~HcalMonitorModule()
{
  ...
  if (dbe_!=0)
    {    
     ...
     if (h2blMon_!=0)  {  h2blMon_->clearME();}   // Testbeam
     if (h2pedMon_!=0) {  h2pedMon_->clearME();}  // Testbeam
     if (h2rhMon_!=0)  {  h2rhMon_->clearME();}   // Testbeam
     if (h2trigMon_!=0) {  h2trigMon_->clearME();}   // Testbeam
  ...
  if (h2blMon_!=0) // Testbeam
    {
      delete h2blMon_; h2blMon_=0;
    }
  if (h2pedMon_!=0) // Testbeam
    {
      delete h2pedMon_; h2pedMon_=0;
    }
  if (h2rhMon_!=0) // Testbeam
    {
      delete h2rhMon_; h2rhMon_=0;
    }
  if (h2trigMon_!=0) // Testbeam
    {
      delete h2trigMon_; h2trigMon_=0;
    }
...
void HcalMonitorModule::endJob(void) {
  ...
  if (h2blMon_!=NULL) h2blMon_->done();   // Testbeam
  if (h2pedMon_!=NULL) h2pedMon_->done(); // Testbeam
  if (h2rhMon_!=NULL) h2rhMon_->done();   // Testbeam
  if (h2trigMon_!=NULL) h2trigMon_->done();   // Testbeam
...
void HcalMonitorModule::reset(){
  ...
  if(h2blMon_!=NULL) h2blMon_->reset();   // Testbeam
  if(h2pedMon_!=NULL) h2pedMon_->reset(); // Testbeam
  if(h2rhMon_!=NULL) h2rhMon_->reset();   // Testbeam
  if(h2trigMon_!=NULL) h2trigMon_->reset();   // Testbeam
  ////////////////////////////////////////////////////
...
void HcalMonitorModule::analyze(const edm::Event& e, const edm::EventSetup& eventSetup){
  ...
  bool laserOK_  = true;
  bool tbOK_  = true; // Testbeam

  // try to get testbeam related objects
  ...
  if (!(e.getByType(trigger)))
    {
      tbOK_=false;
      //if (debug_>0)
	LogWarning("HcalMonitorModule")<<"HcalTBTriggerData not available";
    }
  if (!(e.getByType(qadc)))
    {
      tbOK_=false;
      //if (debug_>0) 
	LogWarning("HcalMonitorModule")<<"HcalTBBeamCounters not available"; 
    }
  if (!(e.getByType(epos)))
    {
      tbOK_=false;
      //if (debug_>0) 
	LogWarning("HcalMonitorModule")<<"HcalTBEventPosition not available"; 
    }
  if (!(e.getByType(time)))
    {
      tbOK_=false;
      //if (debug_>0) 
	LogWarning("HcalMonitorModule")<<"HcalTBTiming not available"; 
    }
  ...
  */
  ...
  // H2BeamLineMonitor (Testbeam)
  if (h2blMon_ != NULL && tbOK_) 
    {
      h2blMon_->processEvent(*trigger,*qadc,*epos,*time);
    }
  if (showTiming_)
    {
      cpu_timer.stop();
      if (h2blMon_!=NULL) std::cout <<"TIMER:: H2BEAMLINE MONITOR ->"<<cpu_timer.cpuTime()<<endl;
      cpu_timer.reset(); cpu_timer.start();
    }
    
  // H2PedestalMonitor (Testbeam)
  if (h2pedMon_ != NULL && tbOK_) 
    {
      h2pedMon_->processEvent(*trigger,*hbhe_digi,*ho_digi,*hf_digi);
    }
  if (showTiming_)
    {
      cpu_timer.stop();
      if (h2pedMon_!=NULL) std::cout <<"TIMER:: H2PEDESTAL MONITOR ->"<<cpu_timer.cpuTime()<<endl;
      cpu_timer.reset(); cpu_timer.start();
    }

  // H2RecHitMonitor (Testbeam)
  if (h2rhMon_ != NULL && tbOK_) 
    {
      h2rhMon_->processEvent(*trigger,*hb_hits,*ho_hits,*hf_hits);
    }
  if (showTiming_)
    {
      cpu_timer.stop();
      if (h2rhMon_!=NULL) std::cout <<"TIMER:: H2RECHIT MONITOR ->"<<cpu_timer.cpuTime()<<endl;
      cpu_timer.reset(); cpu_timer.start();
    }

  // H2TriggerMonitor (Testbeam)
  if (h2trigMon_ != NULL && tbOK_) 
    {
      h2trigMon_->processEvent(*trigger);
    }
  if (showTiming_)
    {
      cpu_timer.stop();
      if (h2trigMon_!=NULL) std::cout <<"TIMER:: H2TRIGGER MONITOR ->"<<cpu_timer.cpuTime()<<endl;
      cpu_timer.reset(); cpu_timer.start();
    }
